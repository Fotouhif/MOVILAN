...
class InferenceHelper:
    def __init__(self, dataset='nyu', device='cuda:0'):
        ...
        if dataset == 'nyu':
            ...
            self.saving_factor = 255/10000 
            model = UnetAdaptiveBins.build(n_bins=256, min_val=self.min_depth, max_val=self.max_depth)
            pretrained_path = "./checkpoint/AdaBins.pt"
        elif dataset == 'kitti':
            self.min_depth = 1e-3
            self.max_depth = 80
    ...
    @torch.no_grad()
    def predict_dir_model_nyu(self, test_dir, out_dir):
        os.makedirs(out_dir, exist_ok=True)
        transform = ToTensor()
        all_files = glob.glob(os.path.join(test_dir, "*"))
        self.model.eval()
        for f in tqdm(all_files):
            image_pil = (Image.open(f)).resize((480,640))
            image = np.asarray(image_pil, dtype='float32') / 255.
            image = transform(image).unsqueeze(0).to(self.device)
            print(image.shape)

            centers, final = self.predict(image)
            print("depth_tensor =", final.squeeze())
            # final = final.squeeze().cpu().numpy()
            final = (final * self.saving_factor).astype('uint8')
            basename = os.path.basename(f).split('.')[0]
            save_path = os.path.join(out_dir, basename + ".png")
            print("depth_tensor =", final.squeeze())
            image_depth = Image.fromarray(final.squeeze())
            resized_image = image_depth.resize((300,300), resample=Image.NEAREST)
